...
function statePrompt(player: { money: number; hunger: number }): string {
  ...
  let s = `今のあなたの状態: 所持金 ${money}、空腹度 ${hunger}/100(100に達すると餓死する)。`;
  // 死への恐怖を空腹・困窮に応じて段階的に強める(生存を最優先させる)
  if (hunger >= 80) {
    s += ' 飢えが限界に近い。死がすぐそこに迫っていて心底恐ろしい。何を措いてもまず食料と金を確保しろ。他人や村に施す余裕など一切ない。';
  } else if (hunger >= 55) {
    s += ' かなり空腹だ。早く食べないと危ない。食料と金の確保が最優先で、寄付や施しをしている場合ではない。';
  }
  if (money < 15) {
    s += ' 金がほとんど無い。このままでは食べ物が買えず飢え死にする。寄付や施しなど論外、なりふり構わず稼ぐか、いざとなれば奪ってでも生き延びろ。';
  } else if (money < 40) {
    s += ' 蓄えが心もとない。生きるのに必要な金まで手放してはいけない。';
  }
  // 価値観: 何より生存優先。余裕がある時だけ他人や村に出す
  s +=
    ' あなたは何より自分が生き延びることを最優先する現実的な人間だ。金は厳しい労働で得た命綱で、余裕がある時にしか他人や村の事業に出さない。困っている相手より、まず自分が飢えないことを守る。';
  return s;
}
...
function parseAction(text: string): { text: string; action: ActionTag | null } {
  ...
  if (!KNOWN_ACTIONS.has(kind)) return { text: text.trim(), action: null };
